...
class WatchListViewController: UIViewController {
    ...
    @IBAction func sortButton(_ sender: Any) {
        ...
        if filterArray.count == percentArr.count{
    ...
    func updateWatchList() {
        ...
        Database.database().reference(withPath: "ID/\(self.uid)/watchlist").observe(DataEventType.value, with: { (snapshot) in
...
//            }
            self.stock = postDict
            for (key, value) in self.stock {
                if(self.objectArray.contains(where: {$0.names == key}) != true){
                    self.objectArray.append(onlineStock(names: key, id: value) )
                }
                
            }
            self.filterArray = self.objectArray
        ...
        Alamofire.request(link).responseString(completionHandler: { response in
            ...
            if let value = response.result.value {
                ...
                if valueArr.count > 10 {
                    ...
                    Database.database().reference(withPath: "ID/\(self.uid)/watchlist/\(String(self.filterArray[self.filterArray.count - 1].names))").setValue(self.filterArray[self.filterArray.count - 1].id)
...
extension WatchListViewController: UITableViewDelegate, UITableViewDataSource{
                   ...
                   leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration?
    {
        let closeAction = UIContextualAction(style: .normal, title:  "Delete", handler: { (ac:UIContextualAction, view:UIView, success:(Bool) -> Void) in
                print("OK, marked")
            Database.database().reference(withPath: "ID/\(self.uid)/watchlist/\(String(self.filterArray[indexPath.row].names))").removeValue()
                success(true)
            let alert = UIAlertController(title: "Delete from watchlist", message: "", preferredStyle: .alert)
                        ...
                        let temp = self.total - amount
                            ...
                            stockItemRef.observeSingleEvent(of: .value, with: { (snapshot) in
                                if snapshot.exists(){
                                    ...
                                    print("stockItem=\(String(describing: stockItem))")
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        if filterArray.isEmpty {
            ...
            alert.addAction(UIAlertAction(title: "Yes", style: .default, handler: {
                ...
                let alert = UIAlertController(title: "Success", message: "Added to watchlist", preferredStyle: .alert)
                alert.addAction(UIAlertAction(title: "OK", style: .cancel, handler: nil))
                self.present(alert, animated: true, completion: nil)
